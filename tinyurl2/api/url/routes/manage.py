...
from tinyurl2.database.session import Session, db_session
...
def create_short_url(
    target_url: schemas.URLCreate, db: Session = Depends(db_session)
) -> URL:
...
def change_target_url(
    url_id: str, target_url: schemas.URLCreate, db: Session = Depends(db_session)
...
def delete_short_url(url_id: str, db: Session = Depends(db_session)) -> str:
    try:
        URLManager(db).delete(url_id)
    except NoResultFound:
        raise HTTPException(status_code=404)
...
def get_url(url_id: str, db: Session = Depends(db_session)) -> int:
    try:
